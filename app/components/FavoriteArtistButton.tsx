...
export default function FavoriteArtistButton({
  ...
  const { addFavoriteArtist, removeFavoriteArtist, isArtistFavorite } =
    usePersonalization();
  const favorited = isArtistFavorite(artist.artistId);

  function handleClick(e: React.MouseEvent) {
    e.preventDefault();
    e.stopPropagation();
    if (favorited) {
      removeFavoriteArtist(artist.artistId);
    } else {
  ...
  return (
    <button
      ...
      onClick={handleClick}
      aria-label={
        favorited ? "Remove artist from favorites" : "Add artist to favorites"
      }
